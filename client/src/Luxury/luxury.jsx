...
import img2lux from "../assets/luxury/luxury2.webp";
...
const [wishlist, setWishlist] = useState(() => {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/api/admin/public/products")
      ...
      const luxuryProducts = data
  .filter(p => Number(p.CategoryID) === 6)
  .map((p, index) => ({
    id: p.ProductID,
    name: p.ProductName,
    price: p.Price,
    img: imagePairs[index % imagePairs.length][0],
    hoverImg: imagePairs[index % imagePairs.length][1],
  //   rating: (3.5 + Math.random() * 1.5).toFixed(1),
  //   reviews: Math.floor(10 + Math.random() * 90),
   }));

        setAllProducts(luxuryProducts);
      ...
      <div className="fp-grid">
        {products.map(product => (
          <div className="fp-card" key={product.id}>
            ...
            <Link to={`/Luxury/${product.id}`} className="fp-card__img-wrap">
